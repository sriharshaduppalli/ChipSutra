...
if (-not (Test-Cmd ollama)) {
    if ($InstallOllama -and (Test-Cmd winget)) {
        Write-Host 'Installing Ollama via winget...'
        winget install -e --id Ollama.Ollama --accept-package-agreements --accept-source-agreements
        Write-Host 'Open a NEW PowerShell window, then:'
        Write-Host "  cd $Root"
        Write-Host '  .\setup-native.ps1 -BuildModel -InstallPythonDeps'
        exit 0
    }
    Write-Host 'Ollama not found. Run: .\setup-native.ps1 -InstallOllama'
    Write-Host 'Or install from https://ollama.com/download'
    exit 1
}
...
if ($envText -notmatch 'OLLAMA_URL') {
...
Write-Host 'Terminal 1 - backend (always use 3.12 venv — not default python 3.14):'
Write-Host "  cd $Root\backend"
Write-Host '  .\run-backend.ps1'
Write-Host '  # or: .\.venv\Scripts\Activate.ps1  then  python -m uvicorn server:app --host 0.0.0.0 --port 8001'
Write-Host ''
...
Write-Host 'Note: Verilator sim needs Linux/Docker; AI Generate works natively on Windows.'
