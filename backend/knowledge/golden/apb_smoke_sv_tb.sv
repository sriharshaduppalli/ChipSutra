// Gold reference: procedural APB SETUP+ACCESS smoke with model_reg
`timescale 1ns / 1ps
module apb_smoke_sv_tb;
  ...
  always #5 PCLK = ~PCLK;
  initial begin
    $dumpfile("apb_smoke_sv_tb.vcd"); $dumpvars(0, apb_smoke_sv_tb);
    ...
    for (i = 0; i < 8; i++) begin
      sel = i[1:0];
      PADDR = {sel[1:0], 2'b00}; PWDATA = 32'hA5A50000 | i; PWRITE = 1;
      PSEL = 1; PENABLE = 0; @(posedge PCLK);          // SETUP
      PENABLE = 1; timeout = 0; @(posedge PCLK);       // ACCESS
      while (!PREADY && timeout < 40) begin @(posedge PCLK); timeout++; end
      if (PREADY) model_reg[sel] = PWDATA; else errors++;
      PSEL = 0; PENABLE = 0; PWRITE = 0; @(posedge PCLK);
      PADDR = {sel[1:0], 2'b00}; PWRITE = 0; PSEL = 1; PENABLE = 0;
      @(posedge PCLK); PENABLE = 1; timeout = 0; @(posedge PCLK);
      while (!PREADY && timeout < 40) begin @(posedge PCLK); timeout++; end
      #1; if (PRDATA !== model_reg[sel]) errors++;
      PSEL = 0; PENABLE = 0; @(posedge PCLK);
    end
    if (errors == 0) $display("PASS: apb_smoke_sv_tb");
