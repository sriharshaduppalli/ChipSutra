  ...
  const run = async () => {
    ...
    try {
      const res = await fetch(`${API}/regress/stream`, {
        ...
        body: JSON.stringify({ project_id: project.id, cases, stop_on_fail: false }),
      });
      ...
      while (true) {
        ...
        for (const part of parts) {
          ...
          try {
            ...
            if (event.type === "case_start") setResults((prev) => [...prev, event]);
            else if (event.type === "case_done") setResults((prev) => prev.map((r) => r.index === event.index ? event : r));
  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-6">
      <div className="card-surface w-full max-w-4xl h-[80vh] flex flex-col">
        ...
        <div className="p-4 border-b border-[#1E293B] flex gap-3 items-center">
          <span className="font-mono text-[10px] text-slate-400">SEEDS</span>
          <input value={seeds} onChange={(e) => setSeeds(e.target.value)} className="w-48 bg-[#0B0E14] border border-[#1E293B] px-2 py-1 text-xs font-mono" placeholder="1,2,3" />
          <span className="font-mono text-[10px] text-slate-500 flex-1">{testbenches.length || 1} test(s) × seeds; max 20 runs</span>
        ...
        <div className="flex-1 overflow-auto p-4">
          <table className="w-full font-mono text-xs">
            <thead><tr className="text-slate-500 text-left"><th>Test</th><th>Seed</th><th>Status</th><th>Simulation</th></tr></thead>
            <tbody>
              {results.map((r) => (
          ...
          {!results.length && <div className="text-slate-500 font-mono text-xs">Testbench files are detected by kind/name. Configure seeds and run.</div>}
        </div>
      </div>
    </div>
  );
