    ...
    <div className="min-h-screen grid-bg flex items-center justify-center p-6">
      <div className="w-full max-w-md card-surface p-8 relative">
        ...
        <form onSubmit={submit} className="space-y-4">
          ...
          <input type="email" required placeholder="email" value={email} onChange={e=>setEmail(e.target.value)} className="w-full bg-[#0B0E14] border border-[#1E293B] px-3 py-2.5 text-sm font-mono focus:outline-none focus:border-emerald-500" data-testid="signup-email" />
          <input type="password" required minLength={6} placeholder="password (min 6 chars)" value={password} onChange={e=>setPassword(e.target.value)} className="w-full bg-[#0B0E14] border border-[#1E293B] px-3 py-2.5 text-sm font-mono focus:outline-none focus:border-emerald-500" data-testid="signup-password" />
          <button disabled={busy} className="btn-neon w-full" data-testid="signup-submit">{busy ? "Creating..." : "Create Workspace →"}</button>
        </form>
        <div className="mt-6 text-xs font-mono text-slate-400 text-center">
          Already have an account? <Link to="/login" className="text-emerald-400 hover:underline" data-testid="link-login">Sign in</Link>
        </div>
      </div>
    </div>
  );
